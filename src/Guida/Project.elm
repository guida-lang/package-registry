module Guida.Project exposing
    ( Project(..), ApplicationInfo, Deps, PackageInfo, Exposed(..)
    , encode, decoder
    )

{-| Turn `guida.json` files into data.
...
{-| There are two types of Guida projects, one for applications and another one
for packages. The `guida.json` is different in each case, so they are modeled
...
{-| The contents of an `guida.json` with `"type": "application"`.
-}
...
{-| The contents of an `guida.json` with `"type": "package"`.
-}
...
{-| There are two ways to specify `"exposed-modules"` field in an `guida.json`
...
{-| Turn a `Project` into the JSON that goes in `guida.json`
-}
encode : Project -> E.Value
encode project =
    case project of
        Application { guida, dirs, depsDirect, depsIndirect, testDepsDirect, testDepsIndirect } ->
            E.object
                [ ( "type", E.string "application" )
                , ( "source-directories", E.list E.string dirs )
                , ( "guida-version", Version.encode guida )
                , ( "dependencies"
                  , E.object
        ...
        Package { name, summary, license, version, exposed, deps, testDeps, guida } ->
            E.object
                ...
                , ( "guida-version", Constraint.encode guida )
...
{-| Decode the contents of `guida.json` into a `Project`.
-}
...
applicationDecoder =
    D.map6 ApplicationInfo
        (D.field "guida-version" Version.decoder)
...
packageDecoder =
    D.map8 PackageInfo
        ...
        (D.field "guida-version" Constraint.decoder)
